...
def request(flow: http.HTTPFlow) -> None:
    ...
    if is_domestic2(flow.request.pretty_host, target_country_code="CN"):
        # 直接访问，不走上游代理
        print(f"[{flow.timestamp_start}][{flow.type}][mode:{flow.mode}][DIRECT][{ip} - {flow.request.pretty_host}]")
        ...
        print(f"[{flow.timestamp_start}][{flow.type}][mode:{flow.mode}][PROXY][{ip} - {flow.request.pretty_host}]")
